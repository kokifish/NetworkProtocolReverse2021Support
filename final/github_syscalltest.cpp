...
void getContext(CONTEXT* ctxt) {
    fprintf(stdout,
            "rax: 0x%lx\nrbx: 0x%lx\nrcx: 0x%lx\nrdx: 0x%lx\nrsp: 0x%lx\nrbp: "
            "0x%lx\nrsi: 0x%lx\nrdi: 0x%lx\nr8: 0x%lx\nr9: 0x%lx\n",
            PIN_GetContextReg(ctxt, REG_RAX), PIN_GetContextReg(ctxt, REG_RBX),
            PIN_GetContextReg(ctxt, REG_RCX), PIN_GetContextReg(ctxt, REG_RDX),
            PIN_GetContextReg(ctxt, REG_RSP), PIN_GetContextReg(ctxt, REG_RBP),
            PIN_GetContextReg(ctxt, REG_RSI), PIN_GetContextReg(ctxt, REG_RDI),
            PIN_GetContextReg(ctxt, REG_R8), PIN_GetContextReg(ctxt, REG_R9));
}
...
/*
Retrieves the arguments of the sendto and recvfrom system calls. Dereferences
then increments the bufptr pointer to grab the value at each byte in the buffer.
*/
...
/*
Entry function before system call execution. Checks all system call numbers but
hooks sendto and recvfrom.
*/
...
*/
void syscallExitCallback(THREADID threadIndex,
                         CONTEXT* ctxt,
                         SYSCALL_STANDARD std,
                         void* v) {
...
int Usage() {
    fprintf(stdout,
            "../../../pin -t obj-intel64/syscalltest.so -- sample program");
    return -1;
}
